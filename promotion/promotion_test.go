...
type apiRequesterGetPromotionsCalculation struct {
	mock.Mock
}

func (m *apiRequesterGetPromotionsCalculation) Call(ctx context.Context, method string, path string, secretKey string, header *http.Header, params interface{}, result interface{}) *xendit.Error {
	m.Called(ctx, method, path, secretKey, nil, params, result)
...
func TestGetPromotionsCalculation(t *testing.T) {
	apiRequesterMockObj := new(apiRequesterGetPromotionsCalculation)
	initTesting(apiRequesterMockObj)
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			apiRequesterMockObj.On(
				...
				context.Background(),
				"GET",
				fmt.Sprintf("%s/promotions/calculate?%s", xendit.Opt.XenditURL, tC.data.QueryString()),
				xendit.Opt.SecretKey,
				nil,
				nil,
				&xendit.PromotionCalculation{},
			).Return(nil)

			resp, err := promotion.GetPromotionsCalculation(tC.data)
